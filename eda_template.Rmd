...
```{r}
fit2 = lm(Inf.Risk ~ log_Lgth.of.Sty + Age + log_R.Cul.Rat + R.CX.ray.Rat + log_N.Beds + Med.Sc.Aff + Region + log_Avg.Pat + log_Avg.Nur + log_Pct.Ser.Fac, data = df)
plot(fit2)
summary(fit2)
vif(fit2)
```
...
```{r}
fit3 = lm(Inf.Risk ~ log_Lgth.of.Sty + Age + log_R.Cul.Rat + R.CX.ray.Rat + Med.Sc.Aff + Region + log_Avg.Nur + log_Pct.Ser.Fac, data = df)
plot(fit3)
summary(fit3)
vif(fit3)
```
